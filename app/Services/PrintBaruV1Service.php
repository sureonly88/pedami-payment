...
class PrintBaruV1Service
{
	public function handlePrintBaru($username,$arrRek,$jenisKertas,$Layanan){

        switch ($Layanan) {
            case 'PDAMBJM':
                $this->handlePdamCetak($username,$arrRek,$jenisKertas,$Layanan);
                break;
        }
    }

    private function handlePdamCetak($username,$arrRek,$jenisKertas,$Layanan){
        ...
        for($i=0;$i<sizeof($arrayDivide); $i++){
            ...
            for($j=0;$j<sizeof($arrayDivide[$i]); $j++){
                if($j != $jmlData){
                    $cetakRek .= $this->kertasKasir($username,$arrayDivide[$i][$j],$jenisKertas) . "\n\n" . $spasi;
                }else{
                    $cetakRek .= $this->kertasKasir($username,$arrayDivide[$i][$j],$jenisKertas);
                }
    ...
    private function kertasKasir($username,$Rek,$jenisKertas){
        $c = 1;
    ...
    private function kertasA4($username,$Rek){

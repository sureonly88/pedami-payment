...
<script type="text/javascript">
    ...
    function Kosongkan(){
        ...
        // $("#blok_message").val("");
        // $("#byadmin").val("");
		$('#email').val('');$('#jenis').val('');$('#is_aktif').val('');
    }
    ...
    function LoadData(){
        dtTable = $('#listData').dataTable( {
            ...
            "columns": [
				...
				{ 'data': 'email' },{ 'data': 'jenis' },{ 'data': 'is_aktif' }
    ...
    function getEdit(mId){
        ...
        $.getJSON("{{ url('admin/setup_email/edit') }}/"+mId, function(msg){
            if(msg.status == "Success"){
                ...
                $("#jenis").val(msg.data.jenis).change();

                $('#is_aktif').val(msg.data.is_aktif).change();
    ...
    function confirmDelete(mId){
        $("#isiConfirm").html("Hapus SETUP EMAIL ini?");
...
<div class="box box-primary">
    <div class="box-header">
      <h3 class="box-title">DAFTAR SETUP EMAIL</h3>
    </div>
    <div class="box-body">
        ...
        <table class="table table-bordered table-hover table-striped dataTable" id="listData">
            <thead>
                <tr>
					...
					<th style='min-width: 100px'>EMAIL</th><th style='min-width: 100px'>JENIS</th><th style='min-width: 100px'>STATUS</th>
                </tr>
            </thead>                
            <tbody>
                <tr>
        ...
        <hr/>
        <input type="button" name="tambah" id="tambah" onclick="showDialog()" value="Tambah SETUP EMAIL" class="btn btn-primary btn-sm" />
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Input SETUP EMAIL</h4>
      </div>
      <div class="modal-body">
            <form role="form" id="formData">
				...
				<div class='form-group'><label for='email' >EMAIL</label><input type='text' class='form-control' id='email' placeholder='Enter EMAIL'></div>
                <div class='form-group'>
                    <label for='jenis' >JENIS</label>
                    <select class="form-control" name="jenis" id="jenis" style="width: 100%">
                        <option value="PDAMBJM">Transaksi PDAMBJM</option>
                        <option value="TOPUP">Permintaan Saldo Baru</option>
                    </select>
                </div>
                <div class='form-group'>
                    <label for='is_aktif' >STATUS</label>

                    <select class="form-control" name="is_aktif" id="is_aktif" style="width: 100%">
